...
<x-layouts.app :title="__('Manajemen Laporan')">
                                ...
                                <tr class="hover:bg-slate-800/50 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex gap-2">
                                            ...
                                            <button onclick="openModal({{ $report->id }}, '{{ $report->status->value }}')"
                                                class="group relative px-3 py-1.5 bg-gradient-to-r from-blue-500/10 to-purple-500/10 border border-blue-500/30 hover:border-blue-400/50 rounded-lg text-blue-400 hover:text-blue-300 text-xs font-mono font-semibold transition-all hover:shadow-lg hover:shadow-blue-500/20 flex items-center gap-1.5">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Modal Update Status -->
    <div id="statusModal" class="fixed inset-0 bg-black/30 backdrop-blur-sm hidden items-center justify-center z-50">
        <div class="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-xl shadow-xl max-w-md w-full p-6 border border-blue-500/20">
            ...
            <form id="statusForm" method="POST">
                ...
                <div class="flex gap-3">
                    <button type="submit" class="flex-1 px-6 py-3 bg-gradient-to-r from-blue-600 to-cyan-500 rounded-xl hover:shadow-lg hover:shadow-cyan-500/30 transition font-semibold font-mono text-white font-bold">
                        Simpan
                    </button>
            ...
            fetch(`/admin/reports/${reportId}`)
                ...
                .then(data => {
                    ...
                    content.innerHTML = `
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="bg-slate-800/50 rounded-xl p-4 border border-blue-500/20">
                                <div class="text-xs font-mono text-blue-300 uppercase tracking-wider mb-2">Pelapor</div>
                                <div class="text-white">${escapeHtml(report.user.name)}</div>
                                <div class="text-xs text-slate-400 mt-1">${report.user.email}</div>
                            </div>
                            <div class="bg-slate-800/50 rounded-xl p-4 border border-blue-500/20">
                        ...
                        <div class="flex gap-3 pt-4 border-t border-slate-700/50">
                            <button onclick="openModal(${report.id}, '${report.status}'); closeDetailModal();"
                                class="flex-1 px-6 py-3 bg-gradient-to-r from-blue-600 to-cyan-500 rounded-xl hover:shadow-lg hover:shadow-cyan-500/30 transition font-semibold font-mono text-white font-bold">
                                Update Status
                            </button>
                            <button onclick="closeDetailModal()"
                                class="px-6 py-3 border border-blue-500/30 hover:border-cyan-400/80 rounded-xl hover:bg-blue-500/10 transition-all backdrop-blur-sm text-white font-medium">
                                Tutup
                            </button>
                        </div>
                    `;
